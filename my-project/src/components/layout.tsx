...
export function AppLayout({
  ...
  return (
    <div className="grid min-h-screen grid-cols-[286px_minmax(0,1fr)] bg-gradient-to-br from-slate-50 via-white to-slate-100 max-[1180px]:grid-cols-1">
      <aside className="sticky top-0 h-screen overflow-y-auto border-r border-white/10 bg-gradient-to-b from-slate-950 via-slate-900 to-teal-950 p-6 text-slate-200 shadow-2xl shadow-slate-900/10 max-[1180px]:static max-[1180px]:h-auto max-[720px]:p-4">
        <div className="mb-8 flex min-w-0 items-center gap-4 max-[1180px]:mb-5">
          <div className="flex h-12 w-12 flex-none items-center justify-center rounded-lg bg-gradient-to-br from-teal-300 to-emerald-400 font-black text-teal-950 shadow-lg shadow-teal-500/20">RTB</div>
          <div className="min-w-0">
            <strong className="block truncate text-white">Skills Gap Tool</strong>
            <span className="block truncate text-sm text-slate-400">Kicukiro TVET ICT</span>
          </div>
        </div>
            ...
            <button
              className={`flex min-h-11 min-w-0 items-center gap-3 rounded-lg border px-3.5 py-3 text-left font-black transition duration-200 max-[1180px]:min-w-max ${
                item.key === currentView
                  ? 'border-teal-300/20 bg-white/10 text-white shadow-inner shadow-white/5'
            ...
            >
              <span className={`inline-flex h-9 w-9 flex-none items-center justify-center rounded-lg border ${
                item.key === currentView
                  ...
                  : 'border-white/10 bg-white/10 text-teal-200'
              }`} aria-hidden="true">
                <Icon size={18} />
              </span>
      ...
      <div className="min-w-0">
        <header className="sticky top-0 z-10 flex min-w-0 items-center justify-between gap-5 border-b border-slate-200/80 bg-white/90 px-8 py-5 shadow-sm shadow-slate-200/60 backdrop-blur-xl max-[720px]:items-stretch max-[720px]:px-4">
          <div className="min-w-0">
            <strong className="block truncate text-lg font-black text-slate-950">{user.name}</strong>
            <span className="block truncate text-sm text-slate-500">{user.institution || 'Skills Gap Analysis Tool'}</span>
          </div>
          <div className="flex flex-none items-center gap-3 max-[720px]:items-start">
            <div className="group relative flex justify-end max-[720px]:justify-start">
              <button
                aria-label="Open account menu"
                className="flex h-12 w-12 items-center justify-center rounded-full border border-slate-200 bg-gradient-to-br from-teal-600 to-emerald-600 font-black text-white shadow-md shadow-teal-900/10 ring-4 ring-white transition hover:-translate-y-0.5 hover:shadow-lg focus-visible:outline-none focus-visible:ring-teal-100"
                type="button"
              >
                {initials || <UserRound size={20} />}
              </button>

              <div className="invisible absolute right-0 top-full z-30 mt-3 w-80 translate-y-2 rounded-lg border border-slate-200 bg-white p-3 opacity-0 shadow-2xl shadow-slate-900/15 transition duration-200 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:visible group-focus-within:translate-y-0 group-focus-within:opacity-100 max-[720px]:left-0 max-[720px]:right-auto max-[720px]:w-[min(20rem,calc(100vw-2rem))]">
                <div className="flex items-start gap-3 rounded-lg bg-gradient-to-br from-slate-50 to-teal-50 p-3">
                  <div className="flex h-11 w-11 flex-none items-center justify-center rounded-full bg-teal-700 font-black text-white">
                    {initials}
                  </div>
                  <div className="min-w-0">
                ...
                <div className="mt-3 grid gap-2">
                  <button
                    className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-black text-slate-700 transition hover:bg-slate-50 hover:text-teal-800"
                    type="button"
                  ...
                  {canUpdateProfile && (
                    <button
                      className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-black text-slate-700 transition hover:bg-teal-50 hover:text-teal-800"
                      type="button"
