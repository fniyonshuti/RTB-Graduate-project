...
export function AppLayout({
  ...
  return (
    <div className="grid min-h-screen grid-cols-[286px_minmax(0,1fr)] bg-gradient-to-br from-slate-50 via-white to-slate-100 max-[1180px]:grid-cols-1">
      <aside className="sticky top-0 h-screen overflow-y-auto border-r border-white/10 bg-gradient-to-b from-[#1F2937] via-slate-900 to-[#0077B6] p-6 text-slate-200 shadow-2xl shadow-slate-900/10 max-[1180px]:static max-[1180px]:h-auto max-[720px]:p-4">
        <div className="mb-8 flex min-w-0 items-center gap-4 max-[1180px]:mb-5">
          <div className="flex h-14 w-14 flex-none items-center justify-center overflow-hidden rounded-lg border border-white/10 bg-white shadow-lg shadow-[#0077B6]/10">
            <img
          ...
          {visibleItems.map((item) => {
            ...
            <button
              className={`flex min-h-11 min-w-0 items-center gap-3 rounded-lg border px-3.5 py-3 text-left font-black transition duration-200 max-[1180px]:min-w-max ${
                item.key === currentView
                  ? 'border-[#0077B6]/20 bg-white/10 text-white shadow-inner shadow-white/5'
            ...
            >
              <span className={`inline-flex h-9 w-9 flex-none items-center justify-center rounded-lg border ${
                item.key === currentView
                  ? 'border-[#0077B6]/30 bg-[#0077B6]/15 text-[#0077B6]'
                  : 'border-white/10 bg-white/10 text-[#0077B6]'
              }`} aria-hidden="true">
                <Icon size={18} />
              </span>
      ...
      <div className="min-w-0">
        <header className="sticky top-0 z-10 flex min-w-0 items-center justify-between gap-5 border-b border-slate-200/80 bg-white/90 px-8 py-5 shadow-sm shadow-slate-200/60 backdrop-blur-xl max-[720px]:flex-col max-[720px]:items-stretch max-[720px]:gap-3 max-[720px]:px-4">
          <div className="min-w-0">
          ...
          <div className="flex flex-none items-center justify-end gap-3 max-[720px]:w-full max-[720px]:items-start max-[720px]:justify-between">
            <div className="group relative flex justify-end">
              ...
              <div className="invisible absolute right-0 top-full z-30 mt-3 w-80 translate-y-2 rounded-lg border border-slate-200 bg-white p-3 opacity-0 shadow-2xl shadow-slate-900/15 transition duration-200 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:visible group-focus-within:translate-y-0 group-focus-within:opacity-100 max-[720px]:right-0 max-[720px]:w-[min(20rem,calc(100vw-2rem))]">
                ...
                <div className="mt-3 grid gap-2">
                  <button
                    className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-black text-slate-700 transition hover:bg-slate-50 hover:text-[#0077B6]"
                    type="button"
                  ...
                  {canUpdateProfile && (
                    <button
                      className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-black text-slate-700 transition hover:bg-[#0077B6]/10 hover:text-[#0077B6]"
                      type="button"
